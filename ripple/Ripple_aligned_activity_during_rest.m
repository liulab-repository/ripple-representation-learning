...
HFB = load('../data/peri_ripple_HFB_dB.mat');
channel = readtable('../data/Channel_ROI.csv');
dtACC = readtable('../data/dd_subject_performance.csv');
Ripple= readtable('../data/dd_channel_rest_and_overall_RR.csv');
...
wh = [0.08 0.06];
ylms = [repmat([-1 2],4,1); repmat([-1 2],3,1); 
        repmat([-1 2],3,1); repmat([-1 2],3,1);];
subcolors = [nan(7,3);flip( brewermap(numel(ROI_y7_dmn_sub),'OrRd') )];
 
% SubjectLevel = table('size',[0,4],'VariableTypes',{'single','single','string','string'}, ...
%     'VariableNames',{'hfb','hfbg','roiname','subject'});
SubjectLevel =[];
ROILevel = table;
...
Rfile = fullfile(pwd,"R_scripts/robust_Corr_rest_periRhfb_behav.R");
RUN_R_Script('',Rfile)
...
% plt.DATA = [cb.Y7_Default,cb.compNonAdjacent,cb.g_Y7_Default,cb.mean_Rest];
...
% plt.DATA = [cb.DMNmPFC,cb.compNonAdjacent,cb.g_DMNmPFC,cb.mean_Rest];
