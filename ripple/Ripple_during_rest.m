...
[p0]=signrank(dt.testAfterMemory, dt.testBeforeMemory ,'Tail','right') ;
[p1]=signrank(dt.testAfterInfer, dt.testBeforeInfer ,'Tail','right')   ;
...
dtchan = readtable('../data/dd_channel_rest_and_overall_RR.csv');
H = figure(3);clf
...
plt.setFont
plt.savefig('Figures/main_fig3.pdf')
...
for iph = 1:numel(phases)
    phase = phases{iph};
    rr = dtchan.(phase);
    hh=load('../data/hippocampal_channels_view.mat');
    climv=[0.0 0.6];
