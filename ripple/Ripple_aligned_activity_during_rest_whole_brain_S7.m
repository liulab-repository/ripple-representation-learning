...
OverallRR = readtable('../data/dd_subject_rest_and_overall_ripple_rate.csv');
OverallRR = OverallRR(:,{'subject','RR_Rest'});

atlas = readtable('../data/atlas_labels.csv');


atlas = atlas(periRRchan,:);
atlas.hfb = HFB.EEG_ALLEpoch;
atlas.hfbg   = HFB.EEG_ALL_generalAvg;
...
dd = nn;%mean(dd(:,abs(HFB.times)<250),2);
yy = 1:size(dd,1);
...
plt.FontSize=7;
plt.setFont
plt.savefig('Figures/supp_peri-ripple-HFB-S7.pdf')
%%
CH = load('../data/Contact_distribution_S7_DMNmPFC.mat')
%%
figure(CH.H)
