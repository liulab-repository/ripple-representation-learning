...
%% preparing data
% read data
dtSubjACC  = readtable('../data/dd_subject_performance.csv');
dtRipple = readtable("../data/dd_ripple_during_learning_raw.csv");
dtRipple(:,{'rippleAmp','rippleDur'})=[];
...
Rfile = fullfile(pwd,"R_scripts/Learning_ripple_iti_vs_feedback_mixed_model_script.R");
RUN_R_Script('',Rfile)
...
Rfile = fullfile(pwd,"R_scripts/Learning1_ACC_trials_RippleRate.R");
RUN_R_Script('',Rfile)

% model 3, Compare the ripple rates in correct and incorrect response trials within each phase.
Rfile = fullfile(pwd,"R_scripts/Learning_ripple_ACC_phases.R");
RUN_R_Script('',Rfile)
...
% remove outliers
% The Fishers' z-transformed Learn-R is continuous and normally distributed.
...
hl=plt.addScatter(slope,int);hold on
plt.ylim([0.6 1]);  
...
plt.setFont
plt.savefig('Figures/main_figure2_learning.pdf')
%% visualization for supplementary figure 1
H = figure(2); clf
 
plt = myFigure([],[],H);
load('../data/learning_raster.mat')
...
plt.setFont
plt.savefig('Figures/supp_fig1_learning.pdf')
