classdef cPlotTools < handle
    ...
    properties 
        ...
        CM
        X
        Width % maybe use method instead of properties?
        MarkerSize
        FontName
        FontSize
        P
    ...
    methods
        ...
        function obj = cPlotTools(DATA,GROUP,H, enlargeScale)
            %UNTITLED8 Construct an instance of this class
            %   Detailed explanation goes here
            % addpath
%             if nargin<1,obj =obj.myfigure;return;end
            addTools(obj)
...
%             end
            ...
            if ~exist('H','var')||isempty(H),H = obj.myfigure;else,H = obj.myfigure(H);end
            ...
            obj.X = X;
            obj.GROUP= GROUP;
            obj.Colors = brewermap(size(DATA,2),'RdYlGn');
            obj.CM = f_deduction_color(1);
            if size(obj.Colors,1)==1,obj.Colors=[.5 .5 .5];end
            obj.Range = [min(prctile(obj.DATA,5)), max(prctile(obj.DATA,95))];
            obj.MarkerSize = 4;
            obj.FontName = 'Arial';
            obj.FontSize = 8;
            obj.E = obj.f_sem(obj.DATA);
            obj.g = [];
...
%             if ~exist('x','var'),x=1:size(obj.DATA,2);end
...
%             h=text(x,y,0,[f_pValue2flag(P)], 'FontSize',obj.FontSize+2);
...
%             h=lsline;
        ...
        function h=addShadeErrorForGLM(obj,weights,xrange,x,y)
            ...
            [~,dev,stat]= glmfit(x,y,'normal','weights',weights)
            ...
            h=plot(p,'FaceColor',c,'EdgeColor','none');
%             h=fill([newx;flip(newx)],[yci(:,1);flip(yci(:,2))],obj.Colors,'EdgeColor',obj.Colors);
 
        end
        
        function [hs,hl]=addPartialResidScatter(obj,XLIM,varargin)
            error('Discarded!')
            obj.DATA(sum(isnan(obj.DATA),2)>0,:)=[];
            x = obj.DATA(:,1);
            y = obj.DATA(:,2);
            c = obj.DATA(:,3);
            ...
            xr=stat1.resid+median(x);
            yr=stat2.resid+median(y);
%             hs=scatter(xr,yr,varargin{:});
            col = obj.Colors;
            if size(col,1)==1,
                col = repmat(col,numel(xr),1);
            end
            hs=scatter(x,yr,15,col ,'filled','CData',col ,varargin{:});
            b = glmfit(x,yr);r =corr(xr,yr);
            fprintf('r=%.3f, b=%.3f\n',r,b(2));
            ...
            obj.gca = gca;
%             col = plt.Colors
%             s1=scatter(xr,yr,15,plt.Colors,'filled','CData',plt.Colors,'MarkerFaceAlpha',0.015);
        end
...
%             hs=scatter(xr,yr,varargin{:});
            ...
            hl=lsline;
            hl.LineWidth=2;
            hl.XData = [min(xr),max(xr)];
            hold on
            obj.addShadeErrorForGLM(ones(size(xr)),hl.XData,xr,yr)
...
%             varargin = parseCheckValue(obj,varargin,'color',obj.Colors);
        ...
        function savefig(obj,sfilename,PaperType)
            fprintf('saving current figure...')
            outdir = fileparts(sfilename);
%              obj.H.PaperOrientation='landscape';
%              obj.H.PaperType='A3';
%             obj.H.PaperPosition(1:2)=[0 0 ];
               
%                H.PaperOrientation='landscape';
                obj.H.PaperUnits = 'centimeters';
                obj.H.PaperPosition(1:4)=[0 0 18*obj.enlargeScale 18*obj.enlargeScale];
                obj.H.PaperType='A4';

%             addpath('/HDD/lbcode-dev-xzb/externalPackages/export_fig-3.27')
%             export_fig("-tif",'')
            if ~exist(fileparts(sfilename),'dir')&&~isempty(fileparts(sfilename))
                mkdir(fileparts(sfilename));end
            print(obj.H,sfilename,'-dpdf', '-vector')
            savefig(obj.H,strrep(sfilename,'.pdf','.fig'))
            exportgraphics(obj.H,strrep(sfilename,'.pdf','.png'),'Resolution',600)
            fprintf('saved\n')
%             export_fig(sfilename,'-nocrop','-pdf','-painters','-nofontswap','-transparent')
        end
...
%               H = figure('Position',[1400 100 2000 1000],'Color','w'); % A4 size
             ...
             if exist('H','var')&& ishandle(H)
                 H.Units = "centimeters";
                 if ~isequal(H.PaperPosition(3:4),[18 18])
             ...
             H.PaperPosition(1:4)=[0 0 18*obj.enlargeScale 18*obj.enlargeScale];
             H.PaperType='A4';
             figure(H)
        end
...
%             factor(3:4)=loc_facet_new(3:4)./loc_facet(3:4);
%             loc_t = obj.g.title_axe_handle.Position;
%             loc_t = [loc_t(1:2)+factor(1:2), loc_t(3:4).*factor(3:4)];
% 
%             loc_l = obj.g.legend_axe_handle.Position;
%             loc_l = [loc_l(1:2)+factor(1:2), loc_l(3:4).*factor(3:4)];
% 
%             loc_t(loc_t>1)=1; loc_l(loc_l>1)=1;


%             set(obj.g.title_axe_handle,'Position',loc_t);
%             set(obj.g.legend_axe_handle,'Position',loc_l);
%             cloc={};
%             for i = 1:numel(axs)
%                 set(g.(axs{i}),'Position');
%             end            
        end
...
%                     set(nonGrayElec(i),'markeredgecolor','k','',0.5)
                end
            end
        end
